    ...
    def train(self, data_path: str, epochs: Optional[int] = None,
        ...
        try:
            ...
            if resume:
                ...
                # When resuming, only pass minimal arguments
                # The checkpoint contains optimizer state, learning rate, etc.
                train_kwargs = {
                    'resume': True,  # Tell YOLO to resume from checkpoint
                }
            else:
                # Initialize YOLO model using architecture selector
                model = self.selector.load_pretrained_model()

                # For new training, pass all hyperparameters
                train_kwargs = {
                    'data': data_path,
                    'epochs': epochs,
                    'batch': batch_size,
